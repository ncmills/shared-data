...
export function buildSuitability(site: string, root = ROOT) {
  ...
  for (const row of factsRows()) {
    ...
    if (!hit.length) {
      rows.push({ id: row.id, eligible: "unreviewed" });
      continue;
    }
  ...
  return {
    site,
    format: 1,
    note: "CORPUS-M4 rules-only suitability. Generated by scripts/corpus/suitability.ts from profiles/<site>.yaml — never hand-edit. eligible is \"no\" (a hard-exclude rule matched) or \"unreviewed\"; rules never write \"yes\" and there is no fit score until M5. No consumer reads this file yet.",
    ...
    counts: { rows: rows.length, no, unreviewed: rows.length - no, byRule },
    rows,
  };
}
