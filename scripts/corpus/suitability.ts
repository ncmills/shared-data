...
export interface SuitabilityRow {
  ...
  /** scored rows: "fit" (graded) or "utility" (transport, not graded, no fit) */
  class?: "fit" | "utility";
  /** 0–1, scored class-fit rows only */
  fit?: number;
  /** deterministic flags from scores/<site>.json (the other-occasion pitch) */
  flags?: string[];
  /** the first matching rule id (see the file's `rules` map) */
  reason?: string;
  /** every matching rule id, present only when more than one matched */
  rules?: string[];
  provenance?: { tagger: string; model?: string; promptOrRulesVersion: string; scoredAt?: string; runId?: string };
}
...
export function buildSuitability(site: string, root = ROOT) {
  ...
  for (const row of factsRows()) {
    ...
    if (!hit.length) {
      ...
      rows.push(
        sc
          ? { id: row.id, eligible: "scored", class: sc.class, ...(sc.class === "fit" ? { fit: sc.fit } : {}), ...(sc.flags?.length ? { flags: sc.flags } : {}), reason: sc.reason, provenance: sc.provenance }
          : { id: row.id, eligible: "unreviewed" },
      );
      continue;
    }
